...
class UserController extends Controller
{
    ...
    public function listMessage(Request $request)
    {
        ...
        $chatroom = ChatRoom::select(['id', 'thumbnail', 'users_id'])
            ...
            ->with([
                'messages' => fn($q) => $q->select(['messages.user_id', 'messages.chatroom_id', 'messages.message', 'messages.created_at', 'users.id as user_id', 'users.name as user_name'])
        ...
        foreach ($chatroom->messages as $message) {
            ...
            unset($message->user_id, $message->chatroom_id);
        }
